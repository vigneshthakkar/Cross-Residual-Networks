...
weights={
    ...
    'conv3': tf.Variable(tf.random_normal([3,3,32,16])),
    'conv4': tf.Variable(tf.random_normal([3,3,48,16])),
    'conv5': tf.Variable(tf.random_normal([3,3,64,16])),
    'conv6': tf.Variable(tf.random_normal([3,3,80,16])),
    'conv7': tf.Variable(tf.random_normal([3,3,96,16])),
    'conv': tf.Variable(tf.random_normal([3,3,48,48])),
    'fc': tf.Variable(tf.random_normal([112,10]))
}
...
biases={
    ...
    'conv4': tf.Variable(tf.random_normal([16])),
    'conv5': tf.Variable(tf.random_normal([16])),
    'conv6': tf.Variable(tf.random_normal([16])),
    'conv7': tf.Variable(tf.random_normal([16])),
    'conv': tf.Variable(tf.random_normal([48])),
...
def net():
    ...
    conv4=conv(conv3,weights['conv4'],biases['conv4'],2)
    conv41=conv(conv3,weights['conv'],biases['conv'],2)
    conv4=concat(conv4,conv41)
    ...
    avg=tf.nn.avg_pool(conv7,ksize=[1,7,7,1],strides=[1,1,1,1],padding='VALID')
    avg=tf.reshape(avg,[-1,112])
...
with tf.Session() as sess:
    ...
    epochs=50
    batchsize=100
    print('Model 1 : Residual Network')
